...
def main():
    ...
    parser.add_argument(
        "--csv_path",
        type=str,
        default="./ENEIDE-data/v1.0/AMD/paragraphs_test.csv",
    ...
    system_prompt = "Sei un politologo esperto conoscitore della politica Italiana. Il tuo compito è quello di annotare riferimenti a Persone, Luoghi e Organizzazione all'interno di testi storici. Generi risposte strutturate in formato JSON. Non scrivere codice Python."
    ...
    instruction_prompt = """
        Estrai i riferimenti a entità nominate di tipo "persona", "luogo" e "organizzazione" all'interno del testo in input, proveniente dagli scritti del politico italiano Aldo Moro (1916 - 1978).
        Estrai le entità nella risposta utilizzando un formato JSON come nell'esempio fornito.
        Esempio di Input: "Nel luglio Togliatti si trasferì a Mosca dove partecipò al VI congresso dell'Internazionale Comunista."
        Esempio di Output:
        ```json
            {"nome":"Togliatti", "tipo":"persona"},
            {"nome":"Mosca", "tipo":"luogo"},
            {"nome":"Internazionale Comunista", "tipo":"organizzazione"}
        ```
        ---------------------
        Input:
        """
    ...
    for row in tqdm(test_data, desc="Processing rows"):
        ...
        try:
            ...
            for entity in entities:
                if entity["tipo"] in {"persona", "organizzazione", "luogo"}:
